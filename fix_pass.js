...
async function fixPassword() {
    ...
    try {
        ...
        // Check if student with same email exists (can cause confusion)
        const resEtud = await pool.query("SELECT * FROM etudiant WHERE email = $1", [email]);
        if (resEtud.rowCount > 0) {
            console.log(`ATTENTION: Un étudiant existe aussi avec l'email ${email}.`);
        }
