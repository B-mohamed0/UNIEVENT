...
const EventInfo = ({ route, navigation }) => {
  const { eventId, profId: studentId, nom } = route.params;
  ...
  const checkFeedback = async () => {
    try {
      const response = await fetch(`${API_URL}/events/${eventId}/feedback/check/${studentId}?role=PROFESSOR`);
  ...
  useFocusEffect(
    React.useCallback(() => {
      setLoading(true);
      fetch(`${API_URL}/events/detail/${eventId}/${studentId}?role=PROFESSOR`)
        .then((res) => res.json())
        .then((data) => {
          setEvent(data);
          setLoading(false);
  ...
  const handleUnregister = async () => {
    try {
      const response = await fetch(`${API_URL}/prof/events/unregister/${event.participation_id}`, {
        method: "DELETE",
      });
      const data = await response.json();
      if (response.ok) {
        // Rafraîchir les données de l'événement
        const res = await fetch(`${API_URL}/events/detail/${eventId}/${studentId}?role=PROFESSOR`);
  ...
  return (
    <View style={styles.container}>
            ...
            <BlurView intensity={8} tint="light" style={styles.glassWrapper}>
              {event.event_status === "Terminé" ? (
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("ProfInscription", {
                      eventId: eventId,
                      studentId: studentId,
                      nom: nom,
      ...
      {/* NAVBAR FIXE */}
      < ProfNavbar id={studentId} nom={nom} />
