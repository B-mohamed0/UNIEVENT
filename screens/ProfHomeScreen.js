...
export default function HomeScreen({ route, navigation }) {
  ...
  const fetchUpcomingEvents = React.useCallback(async () => {
    try {
      const response = await fetch(`${API_URL_EVENTS}/upcoming/${id}?role=PROFESSOR`);
  ...
  const fetchStats = React.useCallback(async () => {
    try {
      const res = await fetch(`${API_URL_EVENTS}/${id}?role=PROFESSOR`);
  ...
  const fetchUserData = React.useCallback(async () => {
    try {
      const profileRes = await fetch(`${API_URL}/prof/profile/${id}`);
            ...
            <View style={styles.eventContent}>
              ...
              <View style={{ marginTop: 15 }}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate("ProfEventinfo", {
                      eventId: event.id,
                      profId: id,
                      nom,
                    })
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          style={[styles.iconButtonGlass, { backgroundColor: theme.iconBg, borderColor: theme.iconBorder }]}
          onPress={() => navigation.navigate("ProfProfile", { id, nom: userData.name })}
        >
          <BlurView intensity={20} tint={isDarkMode ? "dark" : "light"} style={styles.iconBlur}>
            {userData.photo ? (
          ...
          <View style={styles.actionButtonsContainer}>
            <View style={styles.actionButtonWrapper}>
              <LiquidGlassButton
                ...
                onPress={() => navigation.navigate("ProfEventsscreen", { id, nom })}
                isDarkMode={isDarkMode}
              />
            </View>
            ...
            <View style={styles.actionButtonWrapper}>
              <LiquidGlassButton
              title="Statistiques"
                onPress={() => navigation.navigate("ProfStats", { id, nom })}
                isDarkMode={isDarkMode}
              />
            </View>
          </View>
